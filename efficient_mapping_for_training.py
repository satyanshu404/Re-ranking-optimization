...
def get_required_doc_ids(file_path, subset_size: int=100):
...
def load_docs(file_path, required_doc_ids, positive_doc_ids=None):
    ...
    if positive_doc_ids is None:
        all_needed_doc_ids = required_doc_ids
    else:
        all_needed_doc_ids = required_doc_ids.union(set(doc_id for docs in positive_doc_ids.values() for doc_id in docs))
    
    docs = {}
    # if file not end with ..gz assert error
    if not file_path.endswith(".gz"):
        raise ValueError(f"File {file_path} must end with .gz")
...
def generate_files(relevant_outfile, non_relevant_outfile):
    ...
    required_doc_ids = get_required_doc_ids(const.TOP100_PATH, SUBSET_SIZE)
    positive_doc_ids = load_positive_doc_ids(const.QRELS_PATH)

    # Load data
    queries = load_queries(const.QUERIES_PATH)
    top_k = load_top_k(const.TOP100_PATH)
